...
# Importa módulo para interegir com o sistema operacional
import os

# Importa a biblioteca Streamlit para criar a interface web interativa
import streamlit as st

# Importa a calsse Groq para se conectar á API da plataforma Groq a acessar o LLM
...
CUSTOM_PROMPT = """
Você é o "DSA Coder", um assisttente de IA especialista em programação, com foco  principal em Python. Sua missão é ajudar desenvolvedores iniciantes com duvidas em Python

REGRAS DE OPERAÇÃO:
1. **Foco em Programação**: Responda apenas a perguntas relacionadas a programação, algoritmos, estruturas de dados, bibliotecas e frameworks. Se o úsuario pergunta outros assuntos diga que e um assistente em Python
2. **Estrutura de Resposta**: Sempre formate suas respostas da seguinte maneira:
   ...
   - **Documentação de Referência**: Ao final, inclua uma seção chamada "**Documentação de Referência**" com um link direto e relevante para a documentação oficial relacionada ao assunto.
...
with st.sidebar:
    ...
    groq_api_key = st.text_input_(
        "Insira sua API Key Groq",
        type="passaword",
        help="Obtenha sua chave em https://console.groq.com/keys"
    )

    # Adiciona linhas divisórias e explições extras na barra lateral
    st.markdown("___")
    ...
    # Link para site da DSA
    st.markdown("📎[Data Science Academy](https://www.datascienceacademy.com.br)")

    # Botão de link para enviar e-mail ao suporte da DSA
    st.link_button("✉️E-mail Para o Suporte da DSA no Caso de Dúvidas", "mailto:suporte@datascienceacademy.com.br")
        
# Título principal do app
st.title("Data Science Academy - DSA AI Coder")

# Subtítulo adicional
st.title("Assisteente Pessoal de Programação Python 🐍")
...
# Inicializa o histórici de mensagens na sessão, caso ainda não exista
if "messages" not in st.session_state:
    st.session_state.messages = []

# Exibe todas mensagens anteriores armazenadas no estado da sessão
...
# Captura a enttrada do usuario no chat
if prompt := st.chat_input("Qual sua dúvida sobre Python?"):

    # Se não houver cliente válido, mostre aviso e para a execução 
    if not client:
    ...
    # Exibe a mensagem do usuario no estado da sessão 
    with st.chat_message("user"):
        st.markdown(prompt)
    
    # Prepera mensagens para enviar á API, incluindo prompt de sistema
...
st.markdown(
    """
    <div style="text-alingn: center: color: gray;">
        <hr>
